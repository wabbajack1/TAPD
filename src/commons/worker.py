    ...
    def get_batch(self, mode:str="Progress"):
        ...
        for _ in range(self.batch_size):
            ...
            if done:
                ...
                print(f"Mode {mode}: Worker {self.id} with list len {len(self.data[mode])} Average Score: {np.mean(self.data[mode][-100:])}")
                wandb.log({f"{mode}_Score_{self.env_dict[mode].spec.id}": np.mean(self.data[mode][-100:])}, commit=False)
