        ...
        for row, block in enumerate(self.blocks):
            ...
            if row == 0 or len(self.parentCols) < 1:
                if block.mutliOutputBlock == True:
                    ...
                    #print("mult", y.shape, value.shape)
                else:
                ...
                for c, col in enumerate(self.parentCols):
                    ...
                    if isinstance(currOutput, tuple):
                        ac = currOutput[0]
                        val = currOutput[1]
                        ##print("row", row, col.lastOutputList[row - 1].shape, ac.shape, val.shape)
                    else:
                        pass
                        ##print("row", row, col.lastOutputList[row - 1].shape, currOutput.shape)

                if block.mutliOutputBlock == True:
                    y = block.runActivation(currOutput[0])
                    value = block.runActivation(currOutput[1])
                    #print("mult", y.shape, value.shape)
                else:
                    y = block.runActivation(currOutput)
                    #print("single + ", y.shape)
