...
class Agent:
    ...
    def compress(self, ewc):
        ...
        for batch_states, batch_actions, _ in dataloader:
            ...
            kl_loss = criterion(log_probs_kb.unsqueeze(0), log_probs_active.unsqueeze(0).detach()) + self.ewc_loss
    ...
    def compress_training(self, max_frames, task_list):
        frame_idx = 0
        last_saved_frame_idx = 0
        data_value_kb = []
        data_value_ac = []
        
        if self.ewc_flag and task_list is not None:
            ewc = EWC(task_list, self.kb_model, self.device)
        else:
            ewc = None
